...
import dk.lundogbendsen.dao.CarDaoImpl;
...
//			System.out.println(personPager);
		...
		StringUtil.prettyPrintHeadline("Testing CarDAO");
		CarDaoImpl carDAO = new CarDaoImpl();
		carDAO.setEntityManager(entityManager);
		
		entityManager.getTransaction().begin();
		
		carDAO.createCar(new Car("Austin", "Morris Mascot", 20000));
		carDAO.createCar(new Car("Austin", "Morris Minor", 25000));
		carDAO.createCar(new Car("BMW", "Morris Mini Cooper", 275000));
		
		entityManager.getTransaction().commit();
		
		long carCount = carDAO.countCars();
		System.out.println("\nNo of cars: " + carCount);
		...
		System.out.println("\nCars manufactored by Austin are:");
		List<Car> carsByManufactor = carDAO.getCarsByManufactor("Austin");
		for (Car car : carsByManufactor) {
			System.out.println(car);
		}
	}
}