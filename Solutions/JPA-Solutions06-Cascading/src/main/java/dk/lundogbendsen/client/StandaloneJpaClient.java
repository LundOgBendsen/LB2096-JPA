...
public class StandaloneJpaClient {
	...
	public static void exerciseWithJPA(JpaUtil.HibernateLoggingConfig loggingConfig, 
		...
		//remove first order (
		List<Order> orders = documentRepository.findAllOrders(0, 1);
		if (orders.size()==1) {
			Order order1 = orders.get(0);
			//remove reference from Invoice to Order to prevent constraint violations
			order1.getInvoice().detachFromOrder();
			documentRepository.removeOrder(order1);
		}

		executing = "after removal of one order: orderRepository.findAllOrders(0, 10)";
		JpaUtil.prettyPrintQueryResult(executing, documentRepository.findAllOrders(0, 10));
		executing = "after removal of one order: invoiceRepository.findAllInvoices(0, 10)";
		JpaUtil.prettyPrintQueryResult(executing, documentRepository.findAllInvoices(0, 10));
		entityManager.getTransaction().commit();
